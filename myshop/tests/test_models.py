...
from myshop.models import Product, Comment, CartItem, Category, ProductPicture, OrderItem, Order, User
...
class CategoryModelTest(TestCase):
    ...
    def setUp(self):
        self.category = Category.objects.create(title='phones', description='Phones')
...
class ProductCommentTest(TestCase):
    ...
    def setUp(self):
        self.category = Category.objects.create(title='phones1', description='Phones')
        self.user = User.objects.create(username='user', password='123456')
        self.product = Product.objects.create(title='Iphone', description='Iphone_x', price=12345, category=Category.objects.get(id=self.category.id), user=User.objects.get(id=self.user.id))


    def test_comment_name_is_id_user_product(self):
        comment = Comment.objects.create(product=self.product, content='123', rate=4, user=self.user)
        expected_comment_name = f"{comment.id} {comment.user} {comment.product}"
        self.assertEquals(expected_comment_name, str(comment))

    def test_comment_quantity_in_product(self):
        Comment.objects.create(product=self.product, content='123', rate=4, user=self.user)
        Comment.objects.create(product=self.product, content='123', rate=4, user=self.user)
        Comment.objects.create(product=self.product, content='123', rate=4, user=self.user)
        Comment.objects.create(product=self.product, content='123', user=self.user, comment_of_reply=Comment.objects.all().first())
...
class ProductModelTest(TestCase):
    ...
    def test_product_rating_with_one_rate(self):
        Comment.objects.create(product=self.product, rate=5, content='5', user=User.objects.get(id=self.user.id))
        expected_product_rating = 5
        self.assertEquals(expected_product_rating, self.product.rating)

    def test_product_rating_with_two_rate(self):
        Comment.objects.create(product=self.product, rate=5, content='5', user=User.objects.get(id=self.user.id))
        Comment.objects.create(product=self.product, rate=3, content='3', user=User.objects.get(id=self.user.id))
...
class CartItemModelTest(TestCase):
    ...
    def test_cart_item_name_is_id_user_product_quantity(self):
        cart_item = CartItem.objects.create(product=self.product, user=self.user, quantity=5)
    ...
    def test_cart_item_price_with_discount(self):
        product = Product.objects.create(title='Iphone', description='Iphone_x', price=1000, category=Category.objects.get(id=self.category.id), user=User.objects.get(id=self.user.id), discount=50)
        cart_item = CartItem.objects.create(product=product, user=self.user, quantity=5)
...
class OrerItemModelTest(TestCase):
    ...
    def test_order_item_name_is_id_user_product_quantity(self):
        order_item = OrderItem.objects.create(product=self.product, quantity=5, order=self.order)
        expected_order_item_name = f"{order_item.id} {order_item.product} - {order_item.quantity}"
...
class OrerModelTest(TestCase):
    ...
    def test_order_name_is_id_user_product_quantity(self):
        ...
        expected_order_name = f"{order.id} {order.user} {order.status} {order.total_price}"
