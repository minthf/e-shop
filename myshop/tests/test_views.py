...
class ProductListViewTest(APITestCase):
    def test_get_product_list_supplier(self):
        ...
        user = User.objects.create(
            username="mint", password="12345", is_supplier=True
        )
        self.client.force_authenticate(user)
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_get_product_list_client(self):
        url = reverse("products-list")
...
class ProductDetailViewTest(APITestCase):
    def setUp(self):
        ...
        self.user3 = User.objects.create(
            username="mint3", password="12345", is_supplier=True
        )
...
class CommentDetailViewTest(APITestCase):
    def setUp(self):
        ...
        self.comment1 = Comment.objects.create(
            user=self.user2, content="123", rate=5, product=self.product
        )
    ...
    def test_get_own_comment(self):
        url = reverse(
            "comment-detail", args=(self.product.id, self.comment1.id)
        )
        self.client.force_authenticate(self.user2)
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_patch_own_comment_rate_with_incorrect_data(self):
        url = reverse(
            "comment-detail", args=(self.product.id, self.comment2.id)
        )
        self.client.force_authenticate(self.user2)
        data = {"content": "321", "rate": 2}
        response = self.client.patch(url, data, format="json")
...
class OrderCreateViewTest(APITestCase):
    def setUp(self):
        ...
        self.user2 = User.objects.create(
            username="mint2", password="12345", is_supplier=True
        )
        self.product = Product.objects.create(
            title="Iphone",
            description="Iphone_x",
        ...
        self.cart_item1 = CartItem.objects.create(
            product=self.product, user=self.user1, quantity=5
        )
        self.cart_item2 = CartItem.objects.create(
            product=self.product, user=self.user1, quantity=5
        )
        self.cart_item3 = CartItem.objects.create(
            product=self.product, user=self.user1, quantity=5
        )
        self.cart_item4 = CartItem.objects.create(
            product=self.product, user=self.user1, quantity=5
        )
...
class CategoriesViewTest(APITestCase):
    ...
    def test_post_categories_client(self):
        url = reverse("categories-list")
        self.client.force_authenticate(self.user2)
        data = {"title": "qwf", "description": "123"}
        response = self.client.post(url, data, format="json")
...
class ProductPictureView(APITestCase):
    def setUp(self):
        ...
        self.picure1 = ProductPicture.objects.create(
            product=self.product, picture="123.jpg"
        )
