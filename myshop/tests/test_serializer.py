...
from myshop.serializers import ProductSerializer, CommentSerializer, CartItemSerializer, CategorySerializer, CartSerializer, ProductCreateSerializer, PictureSerializer, CommentDetailSerializer, OrderItemSerializer, OrderSerializer, CommentPatchSerializer, CartPatchSerializer
...
class CommentSerializerTest(TestCase):
    ...
    def setUp(self):  
        ...
        self.comment1 = Comment.objects.create(product=self.product, content='1', rate=1, user=self.user)
        self.comment2 = Comment.objects.create(product=self.product, content='2', comment_of_reply=self.comment1, user=self.user)
        self.comment3 = Comment.objects.create(product=self.product, content='3', comment_of_reply=self.comment2, user=self.user)
                                ...
                                [
                                    OrderedDict(
                                        [
                                            ("id", self.comment3.id),
                                            ("rate", None),
                                            ("content", f"{self.comment3.content}"),
                                            ("creation_date", f"{self.comment3.creation_date}"),
...
class PictureSerializerTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create(username='user', password='123456')
        self.product = Product.objects.create(title='Iphone', description='Iphone_x', price=12345, category=Category.objects.get(id=self.category.id), user=User.objects.get(id=self.user.id))
        self.picture = ProductPicture.objects.create(product=self.product, picture='123')
...
class CartSerializerTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create(username='user', password='123456')
        self.product = Product.objects.create(title='Iphone', description='Iphone_x', price=12345, category=Category.objects.get(id=self.category.id), user=User.objects.get(id=self.user.id))
        self.cart_item1 = CartItem.objects.create(user=self.user, product=self.product, quantity=1)
        self.cart_item2 = CartItem.objects.create(user=self.user, product=self.product, quantity=3)
